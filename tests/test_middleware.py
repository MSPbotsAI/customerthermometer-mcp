...
from customerthermometer_mcp.__main__ import _build_http_app
from customerthermometer_mcp.config import Settings
from customerthermometer_mcp.server import create_mcp_server, get_client_from_context
...
def test_missing_header_returns_401_with_required_headers_listed():
    ...
    with TestClient(app) as client:
        ...
        body = resp.json()
        assert "X-CustomerThermometer-Api-Key" in body["required_headers"]
        assert "X-CustomerThermometer-Api-Url" in body["required_headers"]


def test_missing_one_of_two_headers_still_returns_401():
    app, _ = _make_app()
    with TestClient(app) as client:
        resp = client.post(
            "/mcp",
            ...
            headers={
                ...
                "X-CustomerThermometer-Api-Key": "test-key",
            },
        )
        assert resp.status_code == 401
...
def test_header_present_reaches_request_context(monkeypatch):
    ...
    from customerthermometer_mcp.server import GatewayTokenMiddleware, _gateway_creds_var
    ...
    async def run():
        scope = {
            ...
            "path": "/mcp",
            "headers": [
                (b"x-customerthermometer-api-key", b"test-key-123"),
                (b"x-customerthermometer-api-url", b"app.customerthermometer.com/api.php"),
            ],
        }
    ...
    asyncio.run(run())
    assert seen["creds"] == ("test-key-123", "app.customerthermometer.com/api.php")
